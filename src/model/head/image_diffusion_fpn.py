...
class Image_Diffusion_FPN(BaseDepthRefine):
    def __init__(
            self,
            up_scale_factor=1,
            inference_steps=20,
        ...
        self.mink_final_width = int(image_width / 4)
        self.mink_final_height = int(image_height / 4)
        self.ap_loc = ap_loc
        self.cross_attention = cross_attention

        # Pre measured number of points
        self.cross_attention_dim = 12

        if self.ap_loc:
            overshot_dim = 127
            pre_measured_dim = 217
            self.mink_final_dim = 8
            self.mink_final_dim_fpn = 2
            refined_channels_in = overshot_dim + pre_measured_dim + self.mink_final_dim_fpn * 4 + 4
            # refined_channels_in = overshot_dim + pre_measured_dim + self.mink_final_dim + 4
        else:
            overshot_dim = 256
            pre_measured_dim = 256
            self.mink_final_dim = 4
            self.mink_final_dim_fpn = 11
            refined_channels_in = overshot_dim + pre_measured_dim + self.mink_final_dim_fpn * 4

        if self.cross_attention:
            overshot_dim -= self.cross_attention_dim

        in_channels = [192, 384, 768, 1536]
        # in_channels_3d = [96, 96, 128, 128]
        in_channels_3d = [1, 1, 1]

        # self.interpolate_dim = (96, 10000)
        ...
        self.interpolate_dim_fpn = (96, 20000)
        ...
        self.mink_embedding_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(mink_inner_features, self.mink_final_dim * self.mink_final_height * self.mink_final_width,
                      device='cuda'),
        )

        self.mink_embedding_layer_fpn = nn.Sequential(
            nn.Linear(self.interpolate_dim_fpn[0] * self.interpolate_dim_fpn[1], mink_inner_features, device='cuda'),
            nn.ReLU(),
            nn.Linear(mink_inner_features, mink_inner_features, device='cuda'),
            nn.ReLU(),
            nn.Linear(mink_inner_features, self.mink_final_dim_fpn * self.mink_final_height * self.mink_final_width,
                      device='cuda'),
        )
        ...
        # self.queries = torch.rand(self.cross_attention_dim, self.decoder_dim).cuda()
    ...
    def forward(self, overshot, premeasured_map, heatmap, data_3d, return_loss=False, **kwargs):
        ...
        final_mink_fpn_out = []
        last_mink_output = []
        fpn_network = FeaturePyramidNetwork([96, 96, 128, 128], 128).cuda()
        for mink_output in mink_output_tmp:  # mink_output_tmp: (batch #, decoded fea #, ...)
            mink_fea_dict = {}
            for i in range(len(mink_output)):  # 4 -> RGB
                # torch.Size([1, 1, 96, 13351])

                f = mink_output[len(mink_output) - i - 1]
                if i == 0:
                    last_mink_output.append(f)

                mink_fea_dict[i] = f.permute(0, 2, 1, 3)

            fpn_out = fpn_network(mink_fea_dict)
            final_mink_fpn_out.append(fpn_out)
        ...
        for batch_idx, (final_mink_fpn, coord_embedding, mesh_coords, last_mink_feature, freq) in enumerate(
                zip(final_mink_fpn_out, xyzxyz_embedding, final_mesh_coords, last_mink_output, freq_level)):

            mink_final_fpn = []
            for final_mink_fpn_elem in final_mink_fpn:
                final_mink_fpn_fea = final_mink_fpn[final_mink_fpn_elem]
                final_mink_fpn_fea = final_mink_fpn_fea.permute(0, 2, 1, 3)

                final_mink_fpn_fea = F.interpolate(final_mink_fpn_fea, size=self.interpolate_dim_fpn, mode='bilinear')

                mink_final = self.mink_embedding_layer_fpn(final_mink_fpn_fea.reshape(-1)) \
                    .view(self.mink_final_dim_fpn, self.mink_final_height, self.mink_final_width)
                mink_final_fpn.append(mink_final)

            mink_final_fpn = torch.cat(mink_final_fpn, dim=0)
            ...
            assert not torch.isnan(mink_final_fpn).any()

            # mink_output = self.mink_embedding_layer(mink_output.reshape(-1)).view(self.mink_final_dim, self.mink_final_height, self.mink_final_width)
            ...
            if self.ap_loc:
                ...
                if self.cross_attention:
                    ...
                    # seq = last_mink_feature.view(1, last_mink_feature.size(0), last_mink_feature.size(1)).cuda()  # seq = (1, x, 96)
                    ...
                    # concat_input = torch.cat((mink_final_fpn, freq_embedding, coord_embedding, mesh_output, logits))
                    concat_input = torch.cat((mink_final_fpn, freq_embedding, coord_embedding, mesh_output, logits))
                else:
                    # concat_input = torch.cat((mink_final_fpn, freq_embedding, coord_embedding, mesh_output))
                    concat_input = torch.cat((mink_final_fpn, freq_embedding, coord_embedding, mesh_output))
                ...
                final_3d_output.append(mink_final_fpn)
    ...
    def msig_loss(self, pred_depth, gt_depth, premeasured_coords):
        ...
        pred_result = []
        gt_result = []
        for batch_idx in range(premeasured_coords.size(0)):
