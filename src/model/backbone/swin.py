...
class SwinTransformer(BaseModule):
    ...
    def forward(self, x):
        ...
        # TODO Need to change
        #  Trick: change x type as half for half mixed precision
        # x = self.patch_embed(x.to(torch.float16), torch.float16)        # print(x.shape)
        x = self.patch_embed(x)  # print(x.shape)
...
def swin_large_naive_l4w722422k():
    net = SwinTransformer(pretrain_img_size=224,
                 ...
                 out_indices=(0, 1, 2, 3),
                 pretrain_style='official',
                 pretrained='model/backbone/swin_large_patch4_window7_224_22k.pth')
    net.init_weights()
    return net
