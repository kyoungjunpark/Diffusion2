...
class Diffusion_Heatmap_Summary(BaseSummary):
    def __init__(self, log_dir, mode, args, loss_name, metric_name):
        assert mode in ['train', 'val', 'test'], \
            "mode should be one of ['train', 'val', 'test'] " \
            "but got {}".format(mode)

        super(Diffusion_Heatmap_Summary, self).__init__(log_dir, mode, args)
    ...
    def update(self, global_step, sample, output):
        ...
        if 'amp_pred' in output.keys():
            return self.update_joint(global_step, sample, output)
    ...
    def update_joint(self, global_step, sample, output):
        if self.loss_name is not None:
            self.loss = np.concatenate(self.loss, axis=0)
            self.loss = np.mean(self.loss, axis=0, keepdims=True)
        ...
        # dep = sample['dep'].detach().data.cpu().numpy()
        amp_heatmap = output['amp_heatmap'].detach().data.cpu().numpy()
        phase_heatmap = output['phase_heatmap'].detach().data.cpu().numpy()

        # heatmap_a = sample['heatmap_a'].detach().data.cpu().numpy()

        amp_pred = output['amp_pred'].detach().data.cpu().numpy()
        phase_pred = output['phase_pred'].detach().data.cpu().numpy()
        ...
        if output['confidence'] is not None:
            ...
            confidence = np.zeros_like(amp_heatmap)
        ...
        if num_summary > self.args.num_summary:
            ...
            overshot = overshot[0:num_summary, :, :, :]
            # dep = dep[0:num_summary, :, :, :]
            amp_heatmap = amp_heatmap[0:num_summary, :, :, :]
            phase_heatmap = phase_heatmap[0:num_summary, :, :, :]

            amp_pred = amp_pred[0:num_summary, :, :, :]
            phase_pred = phase_pred[0:num_summary, :, :, :]
        ...
        overshot = np.clip(overshot, a_min=0, a_max=1.0)
        # dep = np.clip(dep, a_min=0, a_max=self.args.max_depth)
        amp_heatmap = np.clip(amp_heatmap, a_min=0, a_max=self.args.max_depth)
        amp_pred = np.clip(amp_pred, a_min=0, a_max=self.args.max_depth)

        phase_heatmap = np.clip(amp_heatmap, a_min=0, a_max=self.args.max_depth)
        phase_pred = np.clip(phase_pred, a_min=0, a_max=self.args.max_depth)
        ...
        for b in range(0, num_summary):
            overshot_tmp = overshot[b, :, :, :]
            # dep_tmp = dep[b, 0, :, :]
            amp_heatmap_tmp = amp_heatmap[b, 0, :, :]
            amp_pred_tmp = amp_pred[b, 0, :, :]

            phase_heatmap_tmp = phase_heatmap[b, 0, :, :]
            phase_pred_tmp = phase_pred[b, 0, :, :]
            if 'refineddepth' in output.keys():
                refineddepth_tmp = refineddepth[b, 0, :, :]
            confidence_tmp = confidence[b, 0, :, :]
            ...
            # heatmap_tmp = 255.0 * heatmap_tmp / self.args.max_depth
            # pred_tmp = 255.0 * pred_tmp / self.args.max_depth
            amp_heatmap_tmp = 255.0 * amp_heatmap_tmp
            amp_pred_tmp = 255.0 * amp_pred_tmp

            phase_heatmap_tmp = 255.0 * phase_heatmap_tmp
            phase_pred_tmp = 255.0 * phase_pred_tmp
            if 'refineddepth' in output.keys():
                refineddepth_tmp = 255.0 * refineddepth_tmp / self.args.max_depth
            confidence_tmp = 255.0 * confidence_tmp

            # dep_tmp = cm(dep_tmp.astype('uint8'))
            amp_heatmap_tmp = cm(amp_heatmap_tmp.astype('uint8'))
            amp_pred_tmp = cm(amp_pred_tmp.astype('uint8'))

            phase_heatmap_tmp = cm(phase_heatmap_tmp.astype('uint8'))
            phase_pred_tmp = cm(phase_pred_tmp.astype('uint8'))
            ...
            # dep_tmp = np.transpose(dep_tmp[:, :, :3], (2, 0, 1))
            amp_heatmap_tmp = np.transpose(amp_heatmap_tmp[:, :, :3], (2, 0, 1))
            amp_pred_tmp = np.transpose(amp_pred_tmp[:, :, :3], (2, 0, 1))

            phase_heatmap_tmp = np.transpose(phase_heatmap_tmp[:, :, :3], (2, 0, 1))
            phase_pred_tmp = np.transpose(phase_pred_tmp[:, :, :3], (2, 0, 1))
            if 'refineddepth' in output.keys():
                refineddepth_tmp = np.transpose(refineddepth_tmp[:, :, :3], (2, 0, 1))
            confidence_tmp = np.transpose(confidence_tmp[:, :, :3], (2, 0, 1))

            img = np.concatenate((overshot_tmp, amp_pred_tmp, amp_heatmap_tmp, phase_pred_tmp, phase_heatmap_tmp, confidence_tmp), axis=1)
            list_img.append(img)
    ...
    def save(self, epoch, idx, sample, output):
        # Handle joint evaluate
        if 'amp_pred' in output.keys():
            return self.save_joint(epoch, idx, sample, output)
        ...
        with torch.no_grad():
            if self.args.save_result_only:
                ...
                overshot = overshot[0, :, :, :].data.cpu().numpy()
                premeasured_map = premeasured_map[0, :, :, :].data.cpu().numpy()
                ...
                overshot = 255.0 * np.transpose(overshot, (1, 2, 0))
                premeasured_map = 255.0 * np.transpose(premeasured_map, (1, 2, 0))
                ...
                overshot = np.clip(overshot, 0, 256).astype('uint8')
                premeasured_map = np.clip(premeasured_map, 0, 256).astype('uint8')

                # dep = (255.0*cm(dep)).astype('uint8')
                # print("2")
                pred = cm(pred)
                pred = (255.0 * pred).astype('uint8')
                if 'refineddepth' in output.keys():
                ...
                overshot = Image.fromarray(overshot, 'RGB')
                premeasured_map = Image.fromarray(premeasured_map, 'RGB')
                ...
                heatmap = Image.fromarray(heatmap[:, :, :3], 'RGB')
                ...
                path_save_overshot = '{}/01_overshot.png'.format(self.path_output)
                ...
                path_save_premeasured = '{}/00_premeasured.png'.format(self.path_output)
                path_save_pred = '{}/04_pred_final.png'.format(self.path_output)
                ...
                overshot.save(path_save_overshot)
                ...
                premeasured_map.save(path_save_premeasured)
                heatmap.save(path_save_gt)
    ...
    def save_joint(self, epoch, idx, sample, output):
        ...
        with torch.no_grad():
            if self.args.save_result_only:
                ...
                amp_premeasured_map, phase_premeasured_map = output['premeasured_map']

                amp_premeasured_map = amp_premeasured_map.detach()
                phase_premeasured_map = phase_premeasured_map.detach()
                amp_premeasured_map.mul_(self.img_std.type_as(amp_premeasured_map)).add_(
                    self.img_mean.type_as(amp_premeasured_map))

                phase_premeasured_map.mul_(self.img_std.type_as(phase_premeasured_map)).add_(
                    self.img_mean.type_as(phase_premeasured_map))
                ...
                overshot = overshot[0, :, :, :].data.cpu().numpy()
                amp_premeasured_map = amp_premeasured_map[0, :, :, :].data.cpu().numpy()
                phase_premeasured_map = phase_premeasured_map[0, :, :, :].data.cpu().numpy()

                # dep = dep[0, 0, :, :].data.cpu().numpy()

                amp_pred = amp_pred[0, 0, :, :].data.cpu().numpy()
                phase_pred = phase_pred[0, 0, :, :].data.cpu().numpy()
                ...
                if 'refineddepth' in output.keys():
                    refineddepth = refineddepth[0, 0, :, :].data.cpu().numpy()
                amp_heatmap = amp_heatmap[0, 0, :, :].data.cpu().numpy()
                phase_heatmap = phase_heatmap[0, 0, :, :].data.cpu().numpy()

                # depth_map = depth_map[0, 0, :, :].data.cpu().numpy()

                overshot = 255.0 * np.transpose(overshot, (1, 2, 0))
                amp_premeasured_map = 255.0 * np.transpose(amp_premeasured_map, (1, 2, 0))
                phase_premeasured_map = 255.0 * np.transpose(phase_premeasured_map, (1, 2, 0))
                ...
                overshot = np.clip(overshot, 0, 256).astype('uint8')
                amp_premeasured_map = np.clip(amp_premeasured_map, 0, 256).astype('uint8')
                phase_premeasured_map = np.clip(phase_premeasured_map, 0, 256).astype('uint8')

                # dep = (255.0*cm(dep)).astype('uint8')
                # print("2")
                amp_pred = cm(amp_pred)
                amp_pred = (255.0 * amp_pred).astype('uint8')

                phase_pred = cm(phase_pred)
                phase_pred = (255.0 * phase_pred).astype('uint8')

                if 'refineddepth' in output.keys():
                    refineddepth = (255.0 * cm(refineddepth)).astype('uint8')

                pred_gray = (255.0 * pred_gray).astype('uint8')
                amp_heatmap = (255.0 * cm(amp_heatmap)).astype('uint8')
                phase_heatmap = (255.0 * cm(phase_heatmap)).astype('uint8')
                ...
                overshot = Image.fromarray(overshot, 'RGB')
                amp_premeasured_map = Image.fromarray(amp_premeasured_map, 'RGB')
                phase_premeasured_map = Image.fromarray(phase_premeasured_map, 'RGB')

                # dep = Image.fromarray(dep[:, :, :3], 'RGB')
                amp_pred = Image.fromarray(amp_pred[:, :, :3], 'RGB')
                phase_pred = Image.fromarray(phase_pred[:, :, :3], 'RGB')

                if 'refineddepth' in output.keys():
                    refineddepth = Image.fromarray(refineddepth[:, :, :3], 'RGB')
                pred_gray = Image.fromarray(pred_gray)

                amp_heatmap = Image.fromarray(amp_heatmap[:, :, :3], 'RGB')
                phase_heatmap = Image.fromarray(phase_heatmap[:, :, :3], 'RGB')
                ...
                path_save_dep_map = '{}/02_dep_map_simple.png'.format(self.path_output)
                path_save_init = '{}/03_pred_init.png'.format(self.path_output)
                path_save_amp_premeasured = '{}/00_amp_premeasured.png'.format(self.path_output)
                path_save_phase_premeasured = '{}/00_phase_premeasured.png'.format(self.path_output)

                path_save_amp_pred = '{}/04_amp_pred_final.png'.format(self.path_output)
                path_save_phase_pred = '{}/04_phase_pred_final.png'.format(self.path_output)

                path_save_pred_gray = '{}/05_pred_final_gray.png'.format(
                    self.path_output)
                path_save_amp_gt = '{}/06_amp_gt.png'.format(self.path_output)
                path_save_phase_gt = '{}/06_phase_gt.png'.format(self.path_output)
                ...
                overshot.save(path_save_overshot)
                # dep.save(path_save_dep)
                amp_pred.save(path_save_amp_pred)
                phase_pred.save(path_save_phase_pred)

                if 'refineddepth' in output.keys():
                    refineddepth.save(path_save_refineddepth)
                pred_gray.save(path_save_pred_gray)
                amp_premeasured_map.save(path_save_amp_premeasured)
                amp_heatmap.save(path_save_amp_gt)
                phase_premeasured_map.save(path_save_phase_premeasured)
                phase_heatmap.save(path_save_phase_gt)
                # depth_map.save(path_save_dep_map)
                # print('save rgb path is {}'.format(path_save_rgb))
