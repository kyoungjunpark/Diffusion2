...
class Diffusion_Heatmap_Metric(BaseMetric):
    def __init__(self, args):
        ...
        if "A_P_Joint" in args.head_specify:
            self.amp_metric_name = [
                'dB(mean)', 'dB(median)', 'RMSE', 'MAE', 'iRMSE', 'iMAE', 'REL', 'D^1', 'D^2', 'D^3'
            ]
            self.phase_metric_name = [
                'rad(mean)', 'rad(median)', 'RMSE', 'MAE', 'iRMSE', 'iMAE', 'REL', 'D^1', 'D^2', 'D^3'
            ]
    ...
    def evaluate(self, sample, output, mode):
        ...
        with torch.no_grad():
            ...
            # RMSE / MAE / dB
            pred_norm = torch.from_numpy(normalize_data(pred.cpu(), target_range=(-57, -30))).to('cuda')
            heatmap_norm = torch.from_numpy(normalize_data(heatmap.cpu(), target_range=(-57, -30))).to('cuda')
            ...
            result = [dB_mean, dB_median, rmse, mae, irmse, imae, rel, del_1, del_2, del_3]
    ...
    def evaluate_phase(self, sample, output, mode):
        with torch.no_grad():
            ...
            # Do not the normalization for phase
            phase_pred_norm = pred
            phase_heatmap_norm = heatmap
            ...
            result = [rad_mean, rad_median, rmse, mae, irmse, imae, rel, del_1, del_2, del_3]
            result = torch.stack(result)
            result = torch.unsqueeze(result, dim=0).detach()
        return result
    ...
    def evaluate_real_tmp(self, dir_real, real_name, output):
        with torch.no_grad():
            ...
            for idx in range(len(x_real)):
                ...
                mmwave_coord_indices = find_matching_coordinates(x_real[idx], y_real[idx], coord, threshold=0.05)
                ...
                best_tmp_error = 100
                for x_tmp in range(-5, 5):
                    for y_tmp in range(-5, 5):
            ...
            print("MMwave error: ", np.nanmean(mmwave_errors))
            ...
            for min_ratio in range(51):
                for max_ratio in range(11):
                    ...
                    for idx in range(len(x_real)):
                        ...
                        # print(np.min(x_real_norm), np.max(x_real_norm))
                        best_tmp_error = 100
                        for x_tmp in range(-10, 10):
                            for y_tmp in range(-10, 10):
                                predict_rssi = pred[min(max(round(x_real_norm[idx]) + x_tmp, 0), 351)][min(max(round(y_real_norm[idx]) + y_tmp, 0), 705)]
                                error = abs(predict_rssi - med_rssi_real[idx])
                                if error < best_tmp_error:
                                    final_error = error
    ...
    def evaluate_real_2(self, dir_real, real_name, output):
        with torch.no_grad():
            # (352, 706)
            # pred_origin = output['pred'][0, 0, 40:320, 40:600].detach().cpu().numpy()
            pred_origin = output['pred'][0, 0, :, :].detach().cpu().numpy()
