...
def collation_fn(batch):
    ...
    room_info = []
    for sample in batch:
        # Add twice if multiple heatmap exists
        overshot.append(sample['overshot'])
        heatmap.append(sample['heatmap'])
        if 'phase_heatmap' in sample.keys():
            heatmap.append(sample['phase_heatmap'])
        data_3d.append(sample['data_3d'])
        if 'freq_level' in sample.keys():
            freq.append(sample['freq_level'])
        if 'ap_image_coord' in sample.keys():
            ap_coord.append(sample['ap_image_coord'])

        if 'room_info' in sample.keys():
            room_info.append(sample['room_info'])

    coords, feats, labels = list(zip(*data_3d))
    overshot = torch.stack(overshot)
    ...
    freq = torch.stack(freq) if freq else torch.tensor([])
    ap_coord = torch.stack(ap_coord) if ap_coord else torch.tensor([])

    return {'overshot': overshot, 'heatmap': heatmap, 'coords': torch.cat(coords)
        , 'feats': torch.cat(feats), 'labels': torch.cat(labels), 'freq_level': freq, 'ap_coord': ap_coord,
            'room_info': room_info}


def collation_fn_frames(batch):
    '''
    :param batch:
    :return:   coords_batch: N x 4 (x,y,z,batch)
    '''
    ...
    for sample in batch:
        # Add twice if multiple heatmap exists
        for sample_overshot in sample['overshot']:
            overshot.append(sample_overshot)
        for sample_heatmap in sample['heatmap']:
            heatmap.append(sample_heatmap)
        for sample_data_3d in sample['data_3d']:
            data_3d.append(sample_data_3d)

        if 'freq_level' in sample.keys():
            for sample_freq_level in sample['freq_level']:
                freq.append(sample_freq_level)

        if 'room_info' in sample.keys():
            for sample_room_info in sample['room_info']:
                room_info.append(sample_room_info)
    ...
    return {'overshot': overshot, 'heatmap': heatmap, 'coords': torch.cat(coords)
        , 'feats': torch.cat(feats), 'labels': torch.cat(labels), 'freq_level': freq}
...
def collation_fn_eval_all(batch):
    ...
    '''
    overshot = [sample['overshot'] for sample in batch]
    heatmap = [sample['heatmap'] for sample in batch]
    data_3d = [sample['data_3d'] for sample in batch]
    coords, feats, labels, inds_recons = list(zip(*data_3d))
    overshot = torch.stack(overshot)
    heatmap = torch.stack(heatmap)
    ...
    return {'overshot': overshot, 'heatmap': heatmap, 'coords': torch.cat(coords)
        , 'feats': torch.cat(feats), 'labels': torch.cat(labels), 'inds_recons': torch.cat(inds_recons)}
